...
curl -s -X POST "https://api.netlify.com/api/v1/accounts/${NETLIFY_ACCOUNT_SLUG}/env?site_id=${NETLIFY_SITE_ID}" \
  ...
  -d '[{"key":"DATABASE_URL","values":[{"value":"'"$DATABASE_URL"'","context":"production"}]}]'
```
...
curl -s -X PATCH "https://api.netlify.com/api/v1/accounts/${NETLIFY_ACCOUNT_SLUG}/env/DATABASE_URL?site_id=${NETLIFY_SITE_ID}" \
  -H "Authorization: Bearer $NETLIFY_AUTH_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"value":"'"$DATABASE_URL"'","context":"production"}'
```
...
- The `context` field controls which deploy contexts see the variable. Use `"production"` rather
  than `"all"` — the `"all"` context can fail silently or produce errors with certain API
  endpoints. If you need the variable in preview deploys as well, set it separately for each
  context.
