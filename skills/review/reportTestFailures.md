...
SELF_INFLICTED_SUBTYPE: <if SELF_INFLICTED is yes, one of: fix-regression, refactoring-breakage, wrong-assumption. "fix-regression" = a fix attempt during debugging introduced a new failure. "refactoring-breakage" = a refactoring or polish task changed app code (e.g., API format, accessibility markup) without updating tests. "wrong-assumption" = new tests were written with incorrect assumptions about data state or API behavior. Omit if SELF_INFLICTED is no. This sub-type enables targeted process improvements — e.g., "refactoring-breakage" indicates a need for mandatory test updates alongside API changes, while "fix-regression" indicates debugging quality issues.>
...
SELF_INFLICTED_SUBTYPE: <if SELF_INFLICTED is yes, one of: fix-regression, refactoring-breakage, wrong-assumption. Same definition as individual entries. Omit if SELF_INFLICTED is no.>
...
- Self-inflicted failures (count of failures caused by the agent's own fix attempts during the session, from SELF_INFLICTED field — helps measure fix quality. When SELF_INFLICTED_SUBTYPE data is available, include the sub-type breakdown: fix-regression, refactoring-breakage, wrong-assumption)
